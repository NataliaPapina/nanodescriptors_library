...
clean_df = pd.read_csv(r'C:\Users\Public\Documents\nanomaterials_descriptors_library\tests\preprocess_test\clean_df_nt_only_library.csv')
X = clean_df.drop(columns=["viability (%)", "material"])
y = clean_df["viability (%)"].apply(lambda x: np.log2(x+1))
