...
for j in my_df['np_shell_2'].index:
    try:
        [pmg.Composition(i) for i in nanodesclib.classes.assign_class(my_df.loc[j, 'np_shell_2']).consist()]
...
builder = DescriptorDatasetBuilder(dataframe=my_df.loc[:,['np_core','smiles_1', 'smiles_2', 'smiles_3','mri_r2']], formula_col="np_core", smiles_cols=['smiles_1', 'smiles_2', 'smiles_3'])
result = builder.build()
result.to_csv('df_mri_only_lb.csv', index=False)
...
clean_df.to_csv('clean_df_mri_only_lb.csv', index=False)